...
SUITE["symbolic_utils"]["expand_fraction"] = @benchmarkable expand_fraction($(unwrap((a + b) / c)))
SUITE["symbolic_utils"]["_apply_termwise_add"] = @benchmarkable _apply_termwise(identity, $(unwrap(expr_add)))
SUITE["symbolic_utils"]["_apply_termwise_mul"] = @benchmarkable _apply_termwise(identity, $(unwrap(expr_mul)))
SUITE["symbolic_utils"]["_apply_termwise_div"] = @benchmarkable _apply_termwise(identity, $(unwrap(expr_div)))

SUITE["symbolic_utils"]["simplify_complex_add"] = @benchmarkable simplify_complex($(unwrap(Complex{Num}(a + b, 0 * a))))
SUITE["symbolic_utils"]["simplify_complex_real"] = @benchmarkable simplify_complex($(Complex{Num}(a, 0 * a)))

SUITE["symbolic_utils"]["get_independent_simple"] = @benchmarkable get_independent($a + $b + $c, $t)
SUITE["symbolic_utils"]["get_independent_trig"] = @benchmarkable get_independent($(cos(f * t)^2 + a + b), $t)
...
SUITE["exponentials"]["expand_exp_power"] = @benchmarkable expand_exp_power($(unwrap(exp(a)^3)))
SUITE["exponentials"]["expand_exp_power_nested"] = @benchmarkable expand_exp_power($(unwrap(exp(a)^3 + exp(b)^2)))

SUITE["exponentials"]["simplify_exp_products"] = @benchmarkable simplify_exp_products($(unwrap(exp(a) * exp(b))))
SUITE["exponentials"]["simplify_exp_products_multi"] = @benchmarkable simplify_exp_products($(unwrap(exp(3a) * exp(4b))))
...
trig_expr_product = (a * sin(f * t) + b * cos(f * t)) *
...
SUITE["fourier"]["fourier_cos_term_simple"] = @benchmarkable fourier_cos_term($trig_expr_simple, $f, $t)
SUITE["fourier"]["fourier_cos_term_sq"] = @benchmarkable fourier_cos_term($(cos(f * t)^2), $(2f), $t)
SUITE["fourier"]["fourier_sin_term_simple"] = @benchmarkable fourier_sin_term($(sin(f * t)), $f, $t)
SUITE["fourier"]["fourier_cos_term_product"] = @benchmarkable fourier_cos_term($trig_expr_product, $(2f), $t)
SUITE["fourier"]["fourier_cos_term_hard"] = @benchmarkable fourier_cos_term($trig_expr_hard, $f, $t)
...
SUITE["powers"]["drop_powers_high"] = @benchmarkable drop_powers($((a + b)^3 + (a + b)^5), [$a, $b], 4)
...
SUITE["substitution"]["substitute_all_1rule"] = @benchmarkable substitute_all($(a + b), $(a => b))
SUITE["substitution"]["substitute_all_3rules"] = @benchmarkable substitute_all($expr_sub, $rules_medium)
SUITE["substitution"]["substitute_all_vector"] = @benchmarkable substitute_all([$a, $c, $e_var], $rules_medium)
...
SUITE["construction"]["DifferentialEquation_coupled"] = @benchmarkable DifferentialEquation(
    [d($x, $t, 2) + $ω0^2 * $x - $k * $y, d($y, $t, 2) + $ω0^2 * $y - $k * $x] .~ [$F * cos($ω * $t), 0],
    [$x, $y]
)
